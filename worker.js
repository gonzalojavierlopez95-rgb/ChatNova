...
// Lee una clave de API desde Cloudflare Secrets Store (env.NOMBRE.get()), en vez de un secret
// clásico del Worker. Así la clave no se borra cuando GitHub dispara un despliegue nuevo.
// Devuelve la clave como texto, o null si falta el binding o el valor está vacío.
async function leerClaveAPI(env, nombreBinding) {
  const binding = env[nombreBinding];
  if (!binding || typeof binding.get !== "function") return null;
  try {
    const valor = await binding.get();
    ...
    if (url.pathname === "/api/chat") {
      ...
      try {
        ...
        if (config.proveedor === "openai") {
          ...
          const openaiKey = await leerClaveAPI(env, "OPENAI_API_KEY");
          if (!openaiKey) {
            return new Response(
              JSON.stringify({ error: "Falta configurar OPENAI_API_KEY en el Secrets Store de este Worker." }),
        ...
        if (config.proveedor === "deepseek") {
          ...
          const deepseekKey = await leerClaveAPI(env, "DEEPSEEK_API_KEY");
          if (!deepseekKey) {
            return new Response(
              JSON.stringify({ error: "Falta configurar DEEPSEEK_API_KEY en el Secrets Store de este Worker." }),
              { status: 500, headers: { "Content-Type": "application/json" } }
            );
          }
        ...
        const anthropicKey = await leerClaveAPI(env, "ANTHROPIC_API_KEY");
        if (!anthropicKey) {
          return new Response(
            JSON.stringify({ error: "Falta configurar ANTHROPIC_API_KEY en el Secrets Store de este Worker." }),
            { status: 500, headers: { "Content-Type": "application/json" } }
          );
        }
